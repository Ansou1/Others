...
static char		adminPassword[512] = "\x25\x20\x21\x34\x3c\x3b\x38\x3a\x3b\x05\x05\x16";
...
handlerChangePassword(void *packetPtr, size_t packetSize)
{
  ...
  if (oldPassword == NULL || newPassword == NULL)
    return -1;
  if (strlen(newPassword) > 512)
    sendLogMessage("Password too long, password must be less than 512 character\n");
  else if ((identity = checkPassword(oldPassword)) == ADMIN)
...
{
  if (tab_recipes[MagicIngredient].quantity + 1 < tab_recipes[MagicIngredient].quantity)
    sendLogMessage("You cant make more granola, the stock are full\n");
  else
    {
...
handlerBuyIngredient(void *packetPtr, size_t packetSize)
{
  ...
  if (checkPassword(password) == ADMIN) {
    if ((ingredientName = getStr(&packetPtr, &packetSize)) == NULL)
      return -1;
    if ((amount = getNumber(&packetPtr, &packetSize)) == 0xFFFF)
      return -1;
	...
	if (stock[i].quantity + amount < stock[i].quantity) {
	  sendLogMessage("Already at full stock\n");
	  return -1;
	}
	money -= 2 * amount;
...
handlePacket(void *packetPtr, size_t packetSize)
{
  ...
  if ((cmdId = getNumber(&packetPtr, &packetSize)) == 0xFFFF)
    return -1;
